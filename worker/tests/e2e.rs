...
async fn e2e_main_flows() {
    ...
    let len: u64 = redis::cmd("XLEN").arg("rupy:q:default").query_async(&mut c).await.unwrap();
    ...
    let mut found = None;
    for _ in 0..100 {
        let zs: Vec<(String, f64)> = redis::cmd("ZRANGE").arg("rupy:delayed:default")
            .arg(0).arg(-1).arg("WITHSCORES").query_async(&mut c).await.unwrap();
    ...
    assert!(score >= (t0 + 3000) as f64 - 100.0, "score {score} below window");
    assert!(score <= (t0 + 3000 + 4000) as f64, "score {score} above window");
    let _: u64 = redis::cmd("ZREM").arg("rupy:delayed:default").arg(&member)
        .query_async(&mut c).await.unwrap(); // stop the chain
    ...
    assert!(r["error"]["traceback"].as_str().unwrap().contains("ValueError"));
    ...
    assert_eq!(dlq_env["retries"], 2, "DLQ envelope should carry final retries");
    ...
    assert_eq!(reason, "max_retries");
    let dlq_env: serde_json::Value = serde_json::from_str(&efield).unwrap();
    assert_eq!(dlq_env["retries"], 0, "SerializationError must not consume retries");

    idempotency_and_timeouts(&mut c).await;

    // graceful SIGTERM with nothing in flight -> exit 0
    w.signal(libc::SIGTERM);
    assert_eq!(w.wait_code(20), 0, "worker should exit 0 on graceful shutdown");
    drop(w);
    stop_redis();
}
...
async fn idempotency_and_timeouts(c: &mut redis::aio::MultiplexedConnection) {
    ...
    let (id1, e1) = envelope("fx.echo", "default", |v| v["idempotency_key"] = json!(key.clone()));
    xadd(c, "default", &e1.to_string()).await;
    let r1 = wait_result(c, &id1, 10).await;
    assert_eq!(r1["status"], "success");
    let (id2, e2) = envelope("fx.echo", "default", |v| v["idempotency_key"] = json!(key.clone()));
    ...
    let claimed: String = redis::cmd("GET").arg(format!("rupy:idemp:{key}"))
