...
pub fn start_redis() {
    ...
    let _ = Command::new("redis-cli").args(["-p", &PORT.to_string(), "shutdown", "nosave"]).output();
    std::thread::sleep(Duration::from_millis(200));
    let out = Command::new("redis-server")
        .args(["--port", &PORT.to_string(), "--save", "", "--appendonly", "no", "--daemonize", "yes"])
        .output()
        .expect("redis-server spawn");
    assert!(out.status.success(), "redis-server failed: {out:?}");
    for _ in 0..50 {
        let ping = Command::new("redis-cli").args(["-p", &PORT.to_string(), "ping"]).output();
        if ping.map(|o| String::from_utf8_lossy(&o.stdout).contains("PONG")).unwrap_or(false) {
            return;
        }
...
impl Worker {
    pub fn spawn(queues: &str, extra: &[&str]) -> Worker {
        ...
        cmd.current_dir(fixtures_dir())
            ...
            .args(["--io-threads", "8", "--io-concurrency", "32", "--cpu-workers", "2"])
            .args(["--stats-interval", "5", "--log-level", "debug"])
            .args(extra)
            .env("RUPY_EXEC_CMD", format!("python3 {}", fixtures_dir().join("fake_exec.py").display()))
            .stdout(Stdio::null())
    ...
    pub fn signal(&self, sig: i32) {
        unsafe { libc::kill(self.0.id() as i32, sig); }
    }
...
pub fn unique_id() -> String {
    ...
    format!("{:016x}{:016x}", t as u64, (std::process::id() as u64) << 32 | n)
}
...
pub async fn xadd(c: &mut redis::aio::MultiplexedConnection, queue: &str, payload: &str) {
    let _: String = redis::cmd("XADD")
        .arg(format!("rupy:q:{queue}")).arg("*").arg("e").arg(payload)
...
pub async fn wait_result(c: &mut redis::aio::MultiplexedConnection, id: &str, secs: u64) -> Value {
    let deadline = Instant::now() + Duration::from_secs(secs);
    while Instant::now() < deadline {
        let r: Option<String> = redis::cmd("GET").arg(format!("rupy:result:{id}"))
            .query_async(c).await.unwrap();
        if let Some(s) = r {
...
pub async fn wait_dlq(
    c: &mut redis::aio::MultiplexedConnection, queue: &str, needle: &str, secs: u64,
) -> (String, String, String) {
    let deadline = Instant::now() + Duration::from_secs(secs);
    while Instant::now() < deadline {
        let entries: Vec<(String, Vec<String>)> = redis::cmd("XRANGE")
            .arg(format!("rupy:dlq:{queue}")).arg("-").arg("+")
            .query_async(c).await.unwrap();
        for (_sid, kv) in &entries {
            let get = |k: &str| kv.chunks(2).find(|c| c[0] == k).map(|c| c[1].clone()).unwrap_or_default();
            let e = get("e");
...
pub async fn wait_group(c: &mut redis::aio::MultiplexedConnection, queue: &str, secs: u64) {
    ...
    while Instant::now() < deadline {
        let r: redis::RedisResult<redis::Value> = redis::cmd("XINFO")
            .arg("GROUPS").arg(format!("rupy:q:{queue}")).query_async(c).await;
        if r.is_ok() {
            return;
        }
