...
def test_ready_line_and_roundtrip(child):
    resp = child.request({"id": "r1", "task": "add", "args": [2, 3], "kwargs": {}, "soft_timeout_ms": None})
    assert resp == {"id": "r1", "ok": True, "result": 5}

    # kwargs and sequential requests on the same child
    resp = child.request({"id": "r2", "task": "add", "args": [1], "kwargs": {"b": 10}, "soft_timeout_ms": None})
    assert resp == {"id": "r2", "ok": True, "result": 11}


def test_task_exception_reported_not_fatal(child):
    resp = child.request({"id": "e1", "task": "boom", "args": [], "kwargs": {}, "soft_timeout_ms": None})
    assert resp["id"] == "e1"
    ...
    resp = child.request({"id": "e2", "task": "add", "args": [4, 4], "kwargs": {}, "soft_timeout_ms": None})
    assert resp == {"id": "e2", "ok": True, "result": 8}


def test_traceback_truncated_to_8kb(child):
    resp = child.request({"id": "b1", "task": "bigfail", "args": [], "kwargs": {}, "soft_timeout_ms": None})
    assert resp["ok"] is False
...
def test_soft_timeout_interrupts_sleep(child):
    t0 = time.monotonic()
    resp = child.request(
        {"id": "s1", "task": "sleepy", "args": [2], "kwargs": {}, "soft_timeout_ms": 200},
        timeout=10,
    )
    ...
    resp = child.request({"id": "s2", "task": "sleepy", "args": [0.3], "kwargs": {}, "soft_timeout_ms": None})
    assert resp == {"id": "s2", "ok": True, "result": "done"}


def test_retry_response_with_countdown(child):
    resp = child.request({"id": "t1", "task": "retryme", "args": [2.5], "kwargs": {}, "soft_timeout_ms": None})
    assert resp["id"] == "t1"
    assert resp["ok"] is False
...
def test_retry_response_without_countdown(child):
    resp = child.request({"id": "t2", "task": "retryme", "args": [], "kwargs": {}, "soft_timeout_ms": None})
    assert resp["ok"] is False
    assert resp["retry"] is True
...
def test_non_serializable_result_is_serialization_error(child):
    resp = child.request({"id": "u1", "task": "unser", "args": [], "kwargs": {}, "soft_timeout_ms": None})
    assert resp["id"] == "u1"
    assert resp["ok"] is False
    assert resp["error"]["type"] == "SerializationError"

    resp = child.request({"id": "u2", "task": "add", "args": [1, 1], "kwargs": {}, "soft_timeout_ms": None})
    assert resp == {"id": "u2", "ok": True, "result": 2}


def test_async_task_runs_via_asyncio(child):
    resp = child.request({"id": "a1", "task": "aadd", "args": [3, 4], "kwargs": {}, "soft_timeout_ms": None})
    assert resp == {"id": "a1", "ok": True, "result": 7}


def test_task_prints_do_not_corrupt_protocol(child):
    resp = child.request({"id": "n1", "task": "noisy", "args": [], "kwargs": {}, "soft_timeout_ms": None})
    assert resp == {"id": "n1", "ok": True, "result": "quiet"}


def test_unknown_task_is_reported(child):
    resp = child.request({"id": "x1", "task": "nope", "args": [], "kwargs": {}, "soft_timeout_ms": None})
    assert resp["id"] == "x1"
    assert resp["ok"] is False
...
def test_eof_exits_zero(child):
    resp = child.request({"id": "z1", "task": "add", "args": [0, 0], "kwargs": {}, "soft_timeout_ms": None})
