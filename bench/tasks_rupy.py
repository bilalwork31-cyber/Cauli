"""rupy app under test, per PROTOCOL.md section 6.

Same throwaway redis as Celery (port from BENCH_REDIS_PORT, default 6390).
rupy keeps all its keys under rupy:* in db 0; Celery uses db 0 (broker) and
...
- max_retries=0: Celery tasks do not auto retry on exception, so rupy must
...
def cpu_task() -> str:
    """cpu task: runs in a rupy child process (python3 -m rupy._exec)."""
