...
celeryconfig fairness block (production fair, matched to cauli semantics):
- task_acks_late=True: message is acked after execution, like cauli's
...
- worker_prefetch_multiplier=1: no prefetch hoarding; each slot takes one
  message at a time, matching cauli's admission gate. This is the standard
  production setting for long tasks.
- task_ignore_result=False + result backend: both stacks pay one result
  write per task (cauli always writes cauli:result:{id} when store_result).
...
- result_expires=3600 matches cauli result_ttl=3600.
- json serializer both directions (cauli envelopes and results are JSON).
- No task retries configured: benchmark tasks either succeed or fail once,
  matching tasks_cauli.py max_retries=0.
