"""Scenario C rupy app: per PROTOCOL section 6, additive file.

C runs the ASYNC send variant by default (RUPY_VARIANT defaults to "async"
...
@app.task(name="campaign.dispatch", kind="io", queue="dispatch",
          max_retries=0, timeout=120.0, store_result=False)
def dispatch(campaign_id):
    tick = campaign_common_c.dispatch_tick(campaign_id)
    variant = os.environ.get("RUPY_VARIANT", "async")
...
@app.task(name="campaign.send_batch_sync", kind="io",
          max_retries=0, timeout=900.0, store_result=False)
def send_batch_sync(campaign_id, batch):
    return campaign_common_c.send_batch_sync_c(campaign_id, batch)


@app.task(name="campaign.send_batch_async", kind="io",
          max_retries=0, timeout=900.0, store_result=False)
async def send_batch_async(campaign_id, batch):
    return await campaign_common_c.send_batch_async_c(campaign_id, batch)


@app.task(name="persist.drain", kind="io", queue="persist",
          max_retries=0, timeout=300.0, store_result=False)
def persist_drain():
...
@app.task(name="bgfill.ghost_job", kind="io", queue="backfill_heavy",
          max_retries=0, timeout=300.0, store_result=False)
def ghost_job():
...
@app.task(name="webhook.drain", kind="io", queue="webhook_ingest",
          max_retries=0, timeout=120.0, store_result=False)
def webhook_drain():
