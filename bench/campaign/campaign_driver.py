...
def _task_module(stack):
    return importlib.import_module(
        "campaign_celery" if stack == "celery" else "campaign_cauli"
    )
...
def main():
    ...
    ap.add_argument("--tick", type=float, default=campconfig.CONFIG["TICK_SECONDS"])
    ...
    print(
        f"[campaign] seeded n={args.n} pages={args.pages} webhook_rows=500",
        file=sys.stderr,
    )
    ...
    while True:
        ...
        if now - last_progress >= 10.0:
            last_progress = now
            print(
                f"[campaign] t={now - t0:6.1f}s ticks={ticks} {counts}", file=sys.stderr
            )
        ...
        if alive is False:
            if dead_since is None:
                dead_since = now
                print(
                    "[campaign] worker gone; 10s grace for stragglers", file=sys.stderr
                )
    ...
    lat = [
        float(r["sent_at_ms"] - r["enqueued_first_ms"])
        for r in rows
        if r["status"] == "sent" and r["sent_at_ms"] > 0 and r["enqueued_first_ms"] > 0
    ]
    ...
    blob = {
        ...
        "stack": args.stack,
        "cauli_variant": os.environ.get("CAULI_VARIANT", "sync")
        if args.stack == "cauli"
        else None,
        "n": args.n,
        "pages": args.pages,
    ...
    print(
        f"[campaign] {args.scenario} stack={args.stack} status={status} "
