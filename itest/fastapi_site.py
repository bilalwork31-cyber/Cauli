"""The documented cauli.contrib.fastapi setup path, exercised for real by
test_fastapi.py:

    engine = create_async_engine(dsn, ...)
    app = install_sqlalchemy_session(Cauli(...), engine)

The worker runs `cauli-worker --app fastapi_site:app`. Postgres coordinates
come from CAULI_ITEST_PG_* env vars, mirroring the CAULI_ITEST_PG_* threading
django_site/settings.py uses for its own DATABASES dict, so test_fastapi.py
...
from cauli.contrib.fastapi import get_session, install_sqlalchemy_session
...
_appname = os.environ.get("CAULI_ITEST_APPNAME", "cauli-itest-fastapi")
...
app = install_sqlalchemy_session(Cauli(default_queue="fastapi"), _engine)
